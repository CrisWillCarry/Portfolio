      ...
      <div className="container mx-auto">
        <div className="flex flex-col sm:flex-row items-center justify-between">
          ...
          <ul className="flex space-x-2 sm:space-x-4">
            ...
            <li
              className={`text-white text-xs sm:text-lg font-mono font-bold hover:bg-gray-700 px-2 py-1 rounded cursor-pointer ${activeSection === 'background' && 'bg-gray-700'}`}
              onClick={() => scrollToSection('background')}
            >
              {language==='en'?<p>Background</p>:<p>Parcours</p>}
            </li>
            <li
              className={`text-white text-xs sm:text-lg font-mono font-bold hover:bg-gray-700 px-2 py-1 rounded cursor-pointer ${activeSection === 'skills' && 'bg-gray-700'}`}
              onClick={() => scrollToSection('skills')}
            >
              {language==='en'?<p>Skills</p>:<p>Compétences</p>}
            </li>
            <li
              className={`text-white text-xs sm:text-lg font-mono font-bold hover:bg-gray-700 px-2 py-1 rounded cursor-pointer ${activeSection === 'projects' && 'bg-gray-700'}`}
              onClick={() => scrollToSection('projects')}
            >
              {language==='en'?<p>Projects</p>:<p>Projets</p>}
            </li>
            <li
              className={`text-white text-xs  hover:scale-110 border sm:text-lg font-mono font-bold hover:bg-gray-700 px-2 py-1 rounded cursor-pointer ${activeSection === 'contact' && 'bg-gray-700'}`}
